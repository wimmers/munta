...
locale AlphaClosure =
  fixes X and k :: "'s \<Rightarrow> 'c \<Rightarrow> nat" and \<R> and V :: "('c, t) cval set"
...
lemmas set_of_regions_spec = set_of_regions[OF _ _ _ finite, of _ "k l", folded \<R>_def]
lemmas region_cover_spec = region_cover[of X _ "k l", folded \<R>_def]
lemmas region_unique_spec = region_unique[of "\<R> l" X "k l", folded \<R>_def, simplified]
lemmas regions_closed'_spec = regions_closed'[of "\<R> l" X "k l", folded \<R>_def, simplified]

lemma valid_regions_distinct_spec:
  "R \<in> \<R> l \<Longrightarrow> R' \<in> \<R> l \<Longrightarrow> v \<in> R \<Longrightarrow> v \<in> R' \<Longrightarrow> R = R'"
unfolding \<R>_def using valid_regions_distinct
  by auto (drule valid_regions_distinct, assumption+, simp)+
...
lemma closure_constraint_id:
  "\<forall>(x, m)\<in>collect_clock_pairs g. m \<le> real (k l x) \<and> x \<in> X \<and> m \<in> \<nat> \<Longrightarrow> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l(\<lbrace>g\<rbrace>) = \<lbrace>g\<rbrace> \<inter> V"
proof goal_cases
  case 1
  show ?case
  proof auto
    fix v assume v: "v \<in> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l \<lbrace>g\<rbrace>"
    then obtain R where R: "v \<in> R" "R \<in> \<R> l" "R \<inter> \<lbrace>g\<rbrace> \<noteq> {}" unfolding cla_def by auto
    with ccompatible[OF 1, folded \<R>_def] show "v \<in> \<lbrace>g\<rbrace>" unfolding ccompatible_def by auto
    from R show "v \<in> V" unfolding V_def \<R>_def by auto
  next
    fix v assume v: "v \<in> \<lbrace>g\<rbrace>" "v \<in> V"
    with region_cover[of X v "k l", folded \<R>_def] obtain R where "R \<in> \<R> l" "v \<in> R"
      unfolding V_def by auto
    then show "v \<in> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l \<lbrace>g\<rbrace>" unfolding cla_def using v by auto
  qed
qed

lemma closure_id':
  "Z \<noteq> {} \<Longrightarrow> Z \<subseteq> R \<Longrightarrow> R \<in> \<R> l \<Longrightarrow> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z = R"
proof goal_cases
  case 1
  note A = this
  then have "R \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z" unfolding cla_def by auto
  moreover
  { fix R' assume R': "Z \<inter> R' \<noteq> {}" "R' \<in> \<R> l" "R \<noteq> R'"
    with A obtain v where "v \<in> R" "v \<in> R'" by auto
    with \<R>_regions_distinct[OF _ A(3) this(1) R'(2-)] \<R>_def have False by auto
  }
  ultimately show ?thesis unfolding cla_def by auto
...
lemma closure_id:
  "Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z \<noteq> {} \<Longrightarrow> Z \<subseteq> R \<Longrightarrow> R \<in> \<R> l \<Longrightarrow> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z = R"
proof goal_cases
  case 1
  then have "Z \<noteq> {}" unfolding cla_def by auto
  with 1 closure_id' show ?case by blast
...
lemma closure_update_mono:
  "Z \<subseteq> V \<Longrightarrow> set r \<subseteq> X \<Longrightarrow> zone_set (Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z) r \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l(zone_set Z r)"
proof -
  assume A: "Z \<subseteq> V" "set r \<subseteq> X"
  let ?U = "{R \<in> \<R> l. Z \<inter> R \<noteq> {}}"
  from A(1) region_cover_spec  have "\<forall> v \<in> Z. \<exists> R. R \<in> \<R> l \<and> v \<in> R" unfolding V_def by auto
  then have "Z = \<Union> {Z \<inter> R | R. R \<in> ?U}"
  proof (auto, goal_cases)
    case (1 v)
    then obtain R where "R \<in> \<R> l" "v \<in> R" by auto
    moreover with 1 have "Z \<inter> R \<noteq> {}" "v \<in> Z \<inter> R" by auto
    ultimately show ?case by auto
  qed
  then obtain U where U: "Z = \<Union> {Z \<inter> R | R. R \<in> U}" "\<forall> R \<in> U. R \<in> \<R> l" by blast
  { fix R assume R: "R \<in> U"
    { fix v' assume v': "v' \<in> zone_set (Closure\<^sub>\<alpha>\<^sub>,\<^sub>l (Z \<inter> R)) r - Closure\<^sub>\<alpha>\<^sub>,\<^sub>l(zone_set (Z \<inter> R) r)"
      then obtain v where *:
        "v \<in> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l(Z \<inter> R)" "v' = [r \<rightarrow> 0]v"
      unfolding zone_set_def by auto
      with closure_id[of "Z \<inter> R" R] R U(2) have **:
        "Closure\<^sub>\<alpha>\<^sub>,\<^sub>l(Z \<inter> R) = R" "Closure\<^sub>\<alpha>\<^sub>,\<^sub>l(Z \<inter> R) \<in> \<R> l"
      by fastforce+
      with region_set'_id[OF _ *(1) finite _ _ A(2), of "k l" 0, folded \<R>_def, OF this(2)]
      have ***: "zone_set R r \<in> \<R> l" "[r\<rightarrow>0]v \<in> zone_set R r"
      unfolding zone_set_def region_set'_def by auto
      from * have "Z \<inter> R \<noteq> {}" unfolding cla_def by auto
      then have "zone_set (Z \<inter> R) r \<noteq> {}" unfolding zone_set_def by auto
      from closure_id'[OF this _ ***(1)] have "Closure\<^sub>\<alpha>\<^sub>,\<^sub>l(zone_set (Z \<inter> R) r) = zone_set R r"
      unfolding zone_set_def by auto
      with v' **(1) have False by auto
    }
    then have "zone_set (Closure\<^sub>\<alpha>\<^sub>,\<^sub>l (Z \<inter> R)) r \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l(zone_set (Z \<inter> R) r)" by auto
  } note Z_i = this
  from U(1) have "Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z = \<Union> {Closure\<^sub>\<alpha>\<^sub>,\<^sub>l (Z \<inter> R) | R. R \<in> U}" unfolding cla_def by auto
  then have "zone_set (Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z) r = \<Union> {zone_set (Closure\<^sub>\<alpha>\<^sub>,\<^sub>l (Z \<inter> R)) r | R. R \<in> U}"
  unfolding zone_set_def by auto
  also have "\<dots> \<subseteq> \<Union> {Closure\<^sub>\<alpha>\<^sub>,\<^sub>l(zone_set (Z \<inter> R) r) | R. R \<in> U}" using Z_i by auto
  also have "\<dots> = Closure\<^sub>\<alpha>\<^sub>,\<^sub>l \<Union> {(zone_set (Z \<inter> R) r) | R. R \<in> U}" unfolding cla_def by auto
  also have "\<dots> = Closure\<^sub>\<alpha>\<^sub>,\<^sub>l (zone_set (\<Union> {Z \<inter> R| R. R \<in> U}) r)"
  proof goal_cases
    case 1
    have "zone_set (\<Union> {Z \<inter> R| R. R \<in> U}) r = \<Union> {(zone_set (Z \<inter> R) r) | R. R \<in> U}"
    unfolding zone_set_def by auto
    then show ?case by auto
  qed
  finally show "zone_set (Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z) r \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l(zone_set Z r)" using U by simp
qed

lemma SuccI3:
  "R \<in> (\<R> l) \<Longrightarrow> v \<in> R \<Longrightarrow> t \<ge> 0 \<Longrightarrow> (v \<oplus> t) \<in> R' \<Longrightarrow> R' \<in> (\<R> l) \<Longrightarrow> R' \<in> Succ (\<R> l) R"
 apply (intro SuccI2[of "\<R> l" X "k l", folded \<R>_def, simplified])
    apply assumption+
   apply (intro region_unique[of "\<R> l" X "k l", folded \<R>_def, simplified, symmetric])
by assumption+

lemma closure_delay_mono:
  "Z \<subseteq> V \<Longrightarrow> (Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z)\<^sup>\<up> \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l (Z\<^sup>\<up>)"
proof
  fix v assume v: "v \<in> (Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z)\<^sup>\<up>" and Z: "Z \<subseteq> V"
  then obtain u u' t R where A:
    "u \<in> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z" "v = (u \<oplus> t)" "u \<in> R" "u' \<in> R" "R \<in> \<R> l" "u' \<in> Z" "t \<ge> 0"
  unfolding cla_def zone_delay_def by blast
  from A(3,5) have "\<forall> x \<in> X. u x \<ge> 0" unfolding \<R>_def by fastforce
  with region_cover_spec[of v] A(2,7) obtain R' where R':
    "R' \<in> \<R> l" "v \<in> R'"
  unfolding cval_add_def by auto
  with set_of_regions_spec[OF A(5,4), OF SuccI3, of u] A obtain t where t:
    "t \<ge> 0" "[u' \<oplus> t]\<^sub>\<R> l = R'"
  by auto
  with A have "(u' \<oplus> t) \<in> Z\<^sup>\<up>" unfolding zone_delay_def by auto
  moreover from regions_closed'_spec[OF A(5,4)] t have "(u' \<oplus> t) \<in> R'" by auto
  ultimately have "R' \<inter> (Z\<^sup>\<up>) \<noteq> {}" by auto
  with R' show "v \<in> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l (Z\<^sup>\<up>)" unfolding cla_def by auto
qed

lemma region_V: "R \<in> \<R> l \<Longrightarrow> R \<subseteq> V" using V_def \<R>_def region.cases by auto

lemma closure_V:
  "Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z \<subseteq> V"
unfolding cla_def using region_V by auto

lemma closure_V_int:
  "Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z = Closure\<^sub>\<alpha>\<^sub>,\<^sub>l (Z \<inter> V)"
unfolding cla_def using region_V by auto

lemma closure_constraint_mono:
  "Closure\<^sub>\<alpha>\<^sub>,\<^sub>l g = g \<Longrightarrow> g \<inter> (Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z) \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l (g \<inter> Z)"
unfolding cla_def by auto

lemma closure_constraint_mono':
  assumes "Closure\<^sub>\<alpha>\<^sub>,\<^sub>l g = g \<inter> V"
  shows "g \<inter> (Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z) \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l (g \<inter> Z)"
proof -
  from assms closure_V_int have "Closure\<^sub>\<alpha>\<^sub>,\<^sub>l (g \<inter> V) = g \<inter> V" by auto
  from closure_constraint_mono[OF this, of Z] have
    "g \<inter> (V \<inter> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z) \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l (g \<inter> Z \<inter> V)"
  by (metis Int_assoc Int_commute)
  with closure_V[of Z] closure_V_int[of "g \<inter> Z"] show ?thesis by auto
qed

lemma cla_empty_iff:
  "Z \<subseteq> V \<Longrightarrow> Z = {} \<longleftrightarrow> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z = {}"
unfolding cla_def V_def using region_cover_spec by fast

lemma closure_involutive_aux:
  "U \<subseteq> \<R> l \<Longrightarrow> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l \<Union> U = \<Union> U"
unfolding cla_def using valid_regions_distinct_spec by blast

lemma closure_involutive_aux':
  "\<exists> U. U \<subseteq> \<R> l \<and> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z = \<Union> U"
unfolding cla_def by (rule exI[where x = "{R \<in> \<R> l. R \<inter> Z \<noteq> {}}"]) auto

lemma closure_involutive:
  "Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z = Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z"
using closure_involutive_aux closure_involutive_aux' by metis

lemma closure_involutive':
  "Z \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l W \<Longrightarrow> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l W"
unfolding cla_def using valid_regions_distinct_spec by fast

lemma closure_subs:
  "Z \<subseteq> V \<Longrightarrow> Z \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z"
unfolding cla_def V_def using region_cover_spec by fast

lemma cla_mono':
  "Z' \<subseteq> V \<Longrightarrow> Z \<subseteq> Z' \<Longrightarrow> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z'"
by (meson closure_involutive' closure_subs subset_trans)

lemma cla_mono:
  "Z \<subseteq> Z' \<Longrightarrow> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z'"
using closure_V_int cla_mono'[of "Z' \<inter> V" "Z \<inter> V"] by auto
...
lemma step_z_alpha_complete:
  ...
  apply (frule step_z_V)
   apply assumption
  apply (rotate_tac 3)
  apply (drule cla_empty_iff[where l = l'])
  by auto
...
(*
lemma calc:
  "valid_abstraction A X k \<Longrightarrow> Z \<subseteq> V \<Longrightarrow> A \<turnstile> \<langle>l, Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z\<rangle> \<leadsto>\<^bsub>a\<^esub> \<langle>l', Z'\<rangle>
  \<Longrightarrow> \<exists>Z''. A \<turnstile> \<langle>l, Z\<rangle> \<leadsto>\<^bsub>\<alpha>(a)\<^esub> \<langle>l', Z''\<rangle> \<and> Z' \<subseteq> Z''"
proof (cases rule: step_z.cases, assumption, goal_cases)
  case 1
  note A = this
  from A(1) have "\<forall>(x, m)\<in>clkp_set A l. m \<le> real (k l x) \<and> x \<in> X \<and> m \<in> \<nat>"
  by (fastforce elim: valid_abstraction.cases)
  then have "\<forall>(x, m)\<in>collect_clock_pairs (inv_of A l). m \<le> real (k x) \<and> x \<in> X \<and> m \<in> \<nat>"
  unfolding clkp_set_def collect_clki_def inv_of_def by auto
  from closure_constraint_id[OF this] have *: "Closure\<^sub>\<alpha>\<^sub>,\<^sub>l \<lbrace>inv_of A l\<rbrace> = \<lbrace>inv_of A l\<rbrace> \<inter> V" .
  have "(Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z)\<^sup>\<up> \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l (Z\<^sup>\<up>)" using A(2) by (blast intro!: closure_delay_mono)
  then have "Z' \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l (Z\<^sup>\<up> \<inter> {u. u \<turnstile> inv_of A l})"
  using closure_constraint_mono'[OF *, of "Z\<^sup>\<up>"] unfolding ccval_def by (auto simp: Int_commute A(6))
  with A(4,3) show ?thesis by auto
next
  case (2 g a r)
  ...
  from *(1) A(6) have "\<forall>(x, m)\<in>collect_clock_pairs g. m \<le> real (k x) \<and> x \<in> X \<and> m \<in> \<nat>"
  unfolding clkp_set_def collect_clkt_def by fastforce
  from closure_constraint_id[OF this] have ***: "Closure\<^sub>\<alpha>\<^sub>,\<^sub>l \<lbrace>g\<rbrace> = \<lbrace>g\<rbrace> \<inter> V" .
  from *(2) A(6) have ****: "set r \<subseteq> X" unfolding collect_clkvt_def by fastforce
  from closure_constraint_mono'[OF ***, of Z] have
    "(Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z) \<inter> {u. u \<turnstile> g} \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l (Z \<inter> {u. u \<turnstile> g})" unfolding ccval_def
  by (subst Int_commute) (subst (asm) (2) Int_commute, assumption)
  moreover have "zone_set \<dots> r \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l (zone_set (Z \<inter> {u. u \<turnstile> g}) r)" using **** A(2)
  by (intro closure_update_mono, auto)
  ultimately have "Z' \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l (zone_set (Z \<inter> {u. u \<turnstile> g}) r \<inter> {u. u \<turnstile> inv_of A l'})"
  using closure_constraint_mono'[OF **, of "zone_set (Z \<inter> {u. u \<turnstile> g}) r"] unfolding ccval_def
    apply (subst A(5))
    apply (subst (asm) (5 7) Int_commute)
    apply (rule subset_trans)
...
lemma steps_z_alpha_closure_involutive'_aux:
  ...
  \<Longrightarrow> \<exists> W'. A \<turnstile> \<langle>l, W\<rangle> \<leadsto>\<^bsub>a\<^esub> \<langle>l',W'\<rangle> \<and> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z' \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l W'"
proof (induction rule: step_z.induct)
  case A: (step_t_z A l Z)
  let ?Z' = "Z\<^sup>\<up> \<inter> {u. u \<turnstile> inv_of A l}"
  let ?W' = "W\<^sup>\<up> \<inter> {u. u \<turnstile> inv_of A l}"
...
next
  ...
  proof
    ...
    with A(1) have "A \<turnstile> \<langle>l, u'\<rangle> \<rightarrow> \<langle>l',[r\<rightarrow>0]u'\<rangle>" apply (intro step.intros(1)) apply rule by auto
    moreover from * u'(1) have "[r\<rightarrow>0]u' \<in> ?W'" unfolding zone_set_def by auto
    ultimately have "R' \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l ?W'" using *(1) 1(1) 2(1) unfolding cla_def by auto
    with 1(2) show "v \<in> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l ?W'" ..
  qed
  ultimately show ?case by meson
qed
...
abbreviation part'' ("[_]\<^sub>_" [61,61] 61) where "part'' u l \<equiv> part u (\<R> l)"
...
proof (induction rule: step_z.induct)
  ...
  proof
    ...
    then obtain u d where
      "u \<in> Z" and v': "v' = u \<oplus> d" "u \<oplus> d \<turnstile> inv_of A l" "0 \<le> d"
    unfolding zone_delay_def by blast
    with closure_subs[OF A(3)] A(1) obtain u' R where u': "u' \<in> W" "u \<in> R" "u' \<in> R" "R \<in> \<R> l"
    unfolding cla_def by blast
    then have "\<forall>x\<in>X. 0 \<le> u x" unfolding \<R>_def by fastforce
    ...
    by auto
    from regions_closed'_spec[OF R(1,2) \<open>0 \<le> d\<close>] v'(1) have v'2: "v' \<in> [v']\<^sub>l" by simp
    from A(2) have
    ...
    unfolding ccompatible_def ccval_def by auto
    from valid_regions_distinct_spec[OF v'1(2) 1(1) v'2 1(3)] region_unique_spec[OF u'(2,4)]
    have 2: "[v']\<^sub>l = R'" "[u]\<^sub>l = R" by auto
    from set_of_regions_spec[OF u'(4,3)] v'1(1) 2 obtain t where t:
...
next
  ...
  proof
    ...
    from \<open>u \<in> Z\<close> closure_subs[OF A(4)] A(2) obtain u' R where u':
    ...
    proof -
      ...
      have **: "[r\<rightarrow>0]u \<in> [[r\<rightarrow>0]u]\<^sub>l'"
        using \<open>R' \<in> \<R> l'\<close> \<open>v' \<in> R'\<close> region_unique_spec v'(1) by blast
      from * have
        "\<forall>(x, m)\<in>collect_clock_pairs (inv_of A l'). m \<le> real (k l' x) \<and> x \<in> X \<and> m \<in> \<nat>"
        unfolding inv_of_def clkp_set_def collect_clki_def by fastforce
    ...
    from valid_regions_distinct_spec[OF *(3) \<open>R' \<in> \<R> l'\<close> \<open>v' \<in> [[r\<rightarrow>0]u]\<^sub>l'\<close> \<open>v' \<in> R'\<close>]
    have "[[r\<rightarrow>0]u]\<^sub>l' = R'" .
    from region_unique_spec[OF u'(2,4)] have "[u]\<^sub>l = R" by auto
...
end (* End of context for special region notation *)
...
lemma steps_z_alpha_V: "A \<turnstile> \<langle>l, Z\<rangle> \<leadsto>\<^sub>\<alpha>* \<langle>l',Z'\<rangle> \<Longrightarrow> Z \<subseteq> V \<Longrightarrow> Z' \<subseteq> V"
by (induction rule: steps_z_alpha.induct) (auto intro!: closure_V)
...
next
  ...
  have "\<Z> \<subseteq> V"
    by (meson A(1) A(3) A(8) Z'''(2) step_z_V steps_z_alpha_V)
  from closure_subs[OF this] \<open>W''' \<subseteq> \<Z>\<close> have *: "W''' \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l'' \<Z>" by auto
...
lemma steps_z_alpha_closure_involutive'':
  "A \<turnstile> \<langle>l, Z\<rangle> \<leadsto>\<^sub>\<alpha>* \<langle>l',Z'\<rangle> \<Longrightarrow> A \<turnstile> \<langle>l', Z'\<rangle> \<leadsto> \<langle>l'',Z''\<rangle> \<Longrightarrow> valid_abstraction A X k \<Longrightarrow> Z \<subseteq> V
  \<Longrightarrow> \<exists> Z'''. A \<turnstile> \<langle>l, Z\<rangle> \<leadsto>* \<langle>l'',Z'''\<rangle> \<and> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z'' \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z'''"
proof (induction A l Z l' Z' arbitrary: Z'' l'' rule: steps_z_alpha.induct, goal_cases)
  case refl from this(1) show ?case by blast
next
  case A: (2 A l Z l' Z' l'' Z'' Z''a l''a)
  from A(3) obtain \<Z> where Z'': "Z'' = Closure\<^sub>\<alpha>\<^sub>,\<^sub>l \<Z>" "A \<turnstile> \<langle>l', Z'\<rangle> \<leadsto> \<langle>l'',\<Z>\<rangle>" by auto
  from A(2)[OF Z''(2) A(5,6)] obtain Z''' where Z''':
    "A \<turnstile> \<langle>l, Z\<rangle> \<leadsto>* \<langle>l'',Z'''\<rangle>" "Closure\<^sub>\<alpha>\<^sub>,\<^sub>l \<Z> \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l Z'''"
  by auto
  from steps_z_alpha_V[OF A(1,6)] step_z_V[OF Z''(2)] have *: "\<Z> \<subseteq> V" by blast
  from A Z'' have "A \<turnstile> \<langle>l'', Closure\<^sub>\<alpha>\<^sub>,\<^sub>l \<Z>\<rangle> \<leadsto> \<langle>l''a,Z''a\<rangle>" by auto
  from calc[OF A(5) * this] obtain \<Z>' where **:
    "A \<turnstile> \<langle>l'', \<Z>\<rangle> \<leadsto> \<langle>l''a,\<Z>'\<rangle>" "Z''a \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l \<Z>'"
  by auto
  from steps_z_alpha_closure_involutive'_aux[OF this(1) Z'''(2) A(5) *] obtain W' where ***:
    "A \<turnstile> \<langle>l'', Z'''\<rangle> \<leadsto> \<langle>l''a,W'\<rangle>" "Closure\<^sub>\<alpha>\<^sub>,\<^sub>l \<Z>' \<subseteq> Closure\<^sub>\<alpha>\<^sub>,\<^sub>l W'"
  by auto
  with Z'''(1) have "A \<turnstile> \<langle>l, Z\<rangle> \<leadsto>* \<langle>l''a,W'\<rangle>" by (blast intro: steps_z_alt)
  with closure_involutive'[OF **(2)] ***(2) show ?case by blast
qed
...
next
  case 2: (step A l Z l' Z' Z'' a l'' Z''')
  then obtain Z''a where *: "A \<turnstile> \<langle>l', Z''\<rangle> \<leadsto>\<^bsub>\<upharpoonleft>a\<^esub> \<langle>l'',Z''a\<rangle>" "Z''' = Closure\<^sub>\<alpha>\<^sub>,\<^sub>l'' Z''a" by auto
  from steps_z_alpha_closure_involutive'[OF 2(1,2) this(1) 2(5,6)] obtain W''' where W''':
