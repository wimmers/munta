...
fun error scan xs = scan xs handle ABORT msg => raise Fail (msg ());
...
fun this_string s = this (s |> String.explode |> map Char.toString) >> K s;  (*primitive string -- no symbols here!*)
