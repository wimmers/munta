    ...
    mode ->
      nat ->
        bool ->
          string ->
            string ->
              ((int list * int list) * (int dBMEntry list) list) list ->
                (unit -> unit)
...
fun dbm_subset_implb (A1_, A2_, A3_) n =
  (fn ai => fn bia => fn bi =>
...
fun dbm_subset_impl (A1_, A2_, A3_) n =
  (fn ai => fn bi =>
    (fn f_ => fn () => f_ ((check_diag_impla (A1_, A3_) n n ai) ()) ())
      (fn x =>
        (if x then (fn () => true)
          else dbm_subset_implb (A1_, A2_, A3_) n n ai bi)));
    ...
    ((imp_nfoldli bic (fn _ => (fn () => true))
       (fn xc => fn sigma =>
         reset_canonical_upd_impl
           (linordered_cancel_ab_monoid_add_int, uminus_int, heap_int) m sigma m
           xc zero_inta)
       x_a)
    ()) ())
    ...
    imp_for zero_nata (suc n) (fn sigma => (fn () => (not sigma)))
      (fn xc => fn sigma =>
        (fn f_ => fn () => f_
          ...
          (fn xa =>
            (fn () =>
              (less_DBMEntry linorder_int xa (Le zero_inta) orelse sigma))))
      false)
    x;
    ...
    ((imp_nfoldli xs (fn (_, b) => (fn () => (not b)))
       (fn ma => fn (j, b) =>
         (if equal_inta i j then (fn () => (plus_inta j one_int, b))
           else (fn f_ => fn () => f_ ((dbm_subset_impla (A1_, A2_, A3_) n m ma)
                  ()) ())
                  (fn ba =>
...
fun unreachability_checker broadcast bounds automata m num_states num_actions
  ...
  let
    ...
    val pi =
      (fn (a1, a2) =>
        (fn f_ => fn () => f_
          ((imp_for zero_nata (plus_nata m one_nata) (fn a => (fn () => a))
             (fn xc => fn _ =>
               imp_for zero_nata (plus_nata m one_nata) (fn a => (fn () => a))
                 (fn xf => fn _ =>
                   imp_for zero_nata (plus_nata m one_nata)
                     (fn a => (fn () => a))
                     (fn xj => fn _ =>
                       (fn f_ => fn () => f_
                                 ...
                                 (fn xa =>
                                   (fn () =>
                                     (less_eq_DBMEntry (equal_int, linorder_int)
                                       x_i (plus_DBMEntrya
     linordered_cancel_ab_monoid_add_int x xa)))))))
                     true)
                 true)
             true)
          ()) ())
    ...
    val lei =
      dbm_subset_impl (linordered_cancel_ab_monoid_add_int, equal_int, heap_int)
        m;
    val l_0i =
      (fn () =>
                      ...
                      (fn (b, a) =>
                        ...
                        in
                          (case ab of In _ => NONE
                            | Out ac =>
                              (if membera equal_nat broadcast ac
                                then SOME (b, (g, (ac, (mb, la)))) else NONE)
                            | Sil _ => NONE)
                        end)
                      (trans_mapa i j)));
              val trans_in_broad_grouped =
            ...
            compute_upds_impl bounds init combsa
                             ...
                             (upt zero_nata num_actions)
                                  ...
                                  let
                                    val ins_commited =
                                      map_filter
(fn (p, _) => (if not (null (nth (nth inb p) a)) then SOME p else NONE)) pairs;
                                    val always_commited =
                                      less_nat one_nata
(size_list ins_commited);
                                  in
                                    maps (fn p =>
   let
     val outs = nth (nth outa p) a;
              ...
              val bin_trans_impl =
                (fn (la, s) =>
                  ...
                  in
                    (if null pairs
                      then maps (fn a =>
                                  pairs_by_action_impl bounds la s (nth out a)
                                    (nth ina a))
                             (bin_actions broadcast num_actions)
                      else let
                             val in2 =
...
(fst xb) xc)
 ...
 (fn xe =>
   (fn f_ => fn () => f_
     ((down_impl (linordered_cancel_ab_monoid_add_int, equal_int, heap_int) m
        xe)
     ()) ())
     (fn x_c => (fn () => (x_c :: sigma)))))))))
                     [])
            ...
            (fn xa =>
              (fn () =>
                (less_eq_DBMEntry (equal_int, linorder_int) x_i
                  (plus_DBMEntrya linordered_cancel_ab_monoid_add_int x
                    xa)))))))
true)
                                    true)
                                true)
...
true)
                                     ...
                                     (fn xb =>
                                       (fn f_ => fn () => f_
 ((imp_for zero_nata (plus_nata m one_nata) (fn aa => (fn () => aa))
    (fn xc => fn _ =>
      (fn f_ => fn () => f_
...
true)
                             ...
                             ((let
                                 ...
                                 val invsa =
                                   Vector.fromList
                                     (map (fn i =>
    let
      val mb = default_map_of equal_nat [] (snd (snd (nth automata i)));
      val mc = Vector.fromList (map mb (upt zero_nata (num_states i)));
    in
      mc
    end)
                                       (upt zero_nata n_psa));
 ...
 let
   ...
   val (ac, (mb, la)) = ab;
 in
   (case ac of In _ => NONE | Out _ => NONE
     | Sil ad => SOME (b, (g, (ad, (mb, la)))))
 end)
                                       (trans_mapa i j));
                                 val int_trans_from_loc_impl =
                                   (fn p => fn la => fn laa => fn s =>
                                     let
                                       val aa = trans_i_map p la;
                                     in
                                       map_filter
 (fn (b, ab) =>
   let
     ...
     val sa = mk_updsi s f;
   in
     (if bvali (equal_int, linorder_int) s b andalso check_boundedi bounds sa
       then SOME (g, (Internal ad, (r, (list_update laa p lb, sa)))) else NONE)
   end)
 aa
                                     end);
                                 val int_trans_from_vec_impl =
                                   (fn pairs => fn la => fn s =>
                                     maps (fn (p, laa) =>
    int_trans_from_loc_impl p laa la s)
                                       pairs);
                                 val int_trans_from_all_impl =
                                   (fn la => fn s =>
                                     maps (fn p =>
 ...
 let
   ...
   val (ac, (mb, la)) = ab;
 in
   (case ac of In _ => NONE | Out ad => SOME (b, (g, (ad, (mb, la))))
     | Sil _ => NONE)
 end)
                                       (trans_mapa i j));
                                 val trans_in_map =
 ...
 let
   ...
   val (ac, (mb, la)) = ab;
 in
   (case ac of In ad => SOME (b, (g, (ad, (mb, la)))) | Out _ => NONE
     | Sil _ => NONE)
 end)
                                       (trans_mapa i j));
                                 val trans_out_broad_grouped =
 ...
 (fn (b, aa) =>
   let
     ...
     val (ac, (mb, la)) = ab;
   in
     (case ac of In _ => NONE
       | Out ad =>
         (if membera equal_nat broadcast ad then SOME (b, (g, (ad, (mb, la))))
           else NONE)
       | Sil _ => NONE)
   end)
 (trans_mapa i j)));
 ...
 map (map (filter (fn (b, _) => bvali (equal_int, linorder_int) s b))) ina;
                                       val outa =
 map (map (filter (fn (b, _) => bvali (equal_int, linorder_int) s b))) out;
                                     in
                                       (if null pairs
 then maps (fn aa =>
             maps (fn p =>
                    ...
                    in
                      (if null outs then []
                               ...
                               val combs =
                                 make_combs broadcast bounds automata p aa inb;
                               val outsa = map (fn ab => (p, ab)) outs;
                               val combsa =
                                 (if null combs then map (fn x => [x]) outsa
                                   else maps
  (fn x => map (fn ab => x :: ab) combs) outsa);
                               val init = ([], (Broad aa, ([], (la, s))));
                             in
                               compute_upds_impl bounds init combsa
                             end)
                    end)
        ...
        (upt zero_nata num_actions)
             ...
             let
               val ins_commited =
                 map_filter
                   (fn (p, _) =>
                     (if not (null (nth (nth inb p) aa)) then SOME p else NONE))
                   pairs;
               ...
               maps (fn p =>
                      ...
                      in
                        (if null outs then []
                                     ...
                                     ((equal_lista equal_nat ins_commited
 [p] orelse
null ins_commited) andalso
                                       not
 (list_ex (fn (q, _) => equal_nata q p) pairs))
                                 then []
                                 else let
...
compute_upds_impl bounds init combsa
 ...
 get_commited broadcast bounds automata la;
                                       val ina =
 all_actions_by_state broadcast bounds automata num_actions trans_in_map la;
                                       val out =
 all_actions_by_state broadcast bounds automata num_actions trans_out_map la;
                                     in
                                       (if null pairs
 then maps (fn aa => pairs_by_action_impl bounds la s (nth out aa) (nth ina aa))
        (bin_actions broadcast num_actions)
 else let
        val in2 = all_actions_from_vec num_actions trans_in_map pairs;
        val out2 = all_actions_from_vec num_actions trans_out_map pairs;
      in
        maps (fn aa =>
               pairs_by_action_impl bounds la s (nth out aa) (nth in2 aa))
          (bin_actions broadcast num_actions) @
          maps (fn aa =>
                 pairs_by_action_impl bounds la s (nth out2 aa) (nth ina aa))
            (bin_actions broadcast num_actions)
      end)
                                     end);
                                 val int_trans_impl =
 ...
 get_commited broadcast bounds automata la;
                                     in
                                       (if null pairs
 then int_trans_from_all_impl la s else int_trans_from_vec_impl pairs la s)
                                     end);
                                 val trans_impl =
                                   (fn st =>
                                     int_trans_impl st @
                               ...
                               in
                                 (fn ai => fn bi =>
                                   (fn f_ => fn () => f_
                                     ((imp_nfoldli (trans_impl ai)
(fn _ => (fn () => true))
(fn xb => fn sigma =>
  (fn f_ => fn () => f_
    ((v_dbm_impl (linordered_cancel_ab_monoid_add_int, heap_int) m) ()) ())
    (fn x =>
      (fn f_ => fn () => f_
        ((abstr_FW_impl
           (linordered_cancel_ab_monoid_add_int, uminus_int, equal_int,
        ...
        ()) ())
        (fn xa =>
          (fn f_ => fn () => f_ ((pre_reset_list_impl m xa (fst (snd (snd xb))))
            ()) ())
            (fn xc =>
              (fn f_ => fn () => f_
                ((abstr_FW_impl
