              ...
              (((nat * (nat act * (nat * nat))) list) list) list ->
                (int instrc option) list ->
                  bexp ->
                    (int * int) list ->
                      (nat list) list ->
                        int list -> nat -> (unit -> (result option))
  val k :
    nat ->
      nat ->
        nat ->
...
fun pw_impl A_ (B1_, B2_, B3_) keyi copyi lei a_0i fi succsi emptyi =
  (fn () =>
    let
      ...
      val xab = fi xaa ();
    in
      (if not xa andalso xab then (fn () => true)
        else (fn f_ => fn () => f_ (a_0i ()) ())
               (fn xb =>
                 (fn f_ => fn () => f_ ((emptyi xb) ()) ())
                   (fn x_a =>
                     (if x_a then (fn () => false)
                       else (fn f_ => fn () => f_ (a_0i ()) ())
                              (fn xc =>
                                (fn f_ => fn () => f_ ((keyi xc) ()) ())
                                  (fn xd =>
                          ...
                          (fn x_k =>
                            (fn f_ => fn () => f_
                              ((hms_extract
                                 ...
                                 (ht_delete (B1_, B2_, B3_) (heap_list A_)) x_k
                                 a1d)
                              ()) ())
                              (fn a =>
                                (case a
    ...
    imp_for zero_nata (suc ai) (fn a => (fn () => a))
      (fn xb => fn _ =>
        imp_for zero_nata (suc ai) (fn a => (fn () => a))
          (fn xe => fn _ => fn () =>
            let
              val x_f = mtx_get (heap_DBMEntry A3_) (suc n) bia (xb, xe) ();
...
fun dbm_subset_impl (A1_, A2_, A3_) n =
  (fn ai => fn bi => fn () =>
    let
      val x = check_diag_impla (A1_, A3_) n n ai ();
    in
      (if x then (fn () => true)
...
fun reachability_checker_impl p m max_steps inv trans prog bounds pred s_0 na k
  formula =
  let
      ...
      let
        val init_sat =
          (fn f_ => fn () => f_
            ...
            (fn x =>
              (fn f_ => fn () => f_
                ((mtx_new (heap_DBMEntry heap_int) (suc m) (suc m)
                   (fn (il, j) =>
                     (if equal_nata il j orelse
                           (less_nat m il orelse less_nat m j)
                       then Le zero_inta else INF)))
                ()) ())
                (fn xa =>
                  (fn f_ => fn () => f_
                    ((abstr_upd_impl
                    ...
                    val suba =
                      (fn ai => fn bi =>
                        let
                          ...
                          val (a1a, a2a) = bi;
                        in
                          (if equal_proda (equal_list equal_nat)
                                (equal_list equal_int) a1 a1a
...
(op_list_prepend (a2c, xa) sigma))))
                    ...
                    val a =
                      (fn (_, a) =>
                        check_diag_impl
                          (linordered_cancel_ab_monoid_add_int, heap_int) m a);
                  in
                    pw_impl
                      (heap_prod
                 ...
                 ()) ())
                 (fn x => (fn () => (if x then REACHABLE else UNREACHABLE)))
          else (fn () => INIT_INV_ERR))
          ()
      end)
  end;

fun check_and_verify pa m k max_steps i t prog final bounds p s_0 na =
...
end; (*struct Reachability_Checker*)
