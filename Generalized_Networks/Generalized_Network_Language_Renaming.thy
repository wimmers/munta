...
lemma A_urge_split:
  "A_urge = (syncs, map N_urge [0..<n_ps], bounds)"
  unfolding syncs_def N_urge_def bounds_def n_ps_def A_urge_def by (cases A)(simp add: map_nth')

lemma inv_add_inv':
  "inv (add_inv A') =
  (\<lambda>l. if l \<in> urgent A' then acconstraint.LE urge 0 # inv A' l else inv A' l)"
  by (cases A', simp add: inv_add_inv urgent_def) (auto simp: inv_def)
...
lemma clock_set_set':
  "[r\<rightarrow>d]u(x:=d') = [r\<rightarrow>d]u" if "x \<in> set r"
  using that by (metis clock_set.simps(2) clock_set_set clock_set_upd_simp fun_upd_triv)

lemma clock_set_set_cong:
  "[r\<rightarrow>d]u = [r'\<rightarrow>d]u" if "set r = set r'"
  using that by (intro ext) (metis clock_set_id clock_set_set)
  ...
  proof cases
    ...
    moreover from steps(3,2) have
      ...
      subgoal broadcast premises prems[tagged]
        for sync syncs' ps N' as bs gs fs rs ls' B
        ...
        preferT \<open>''committed''\<close>
          apply (tag, simp add: committed_N_urge; fail)
        preferT \<open>''guard''\<close>
          apply (tag "''guard''" "TRANS _"; auto simp: trans_urge_upd_iff; fail)
        ...
        preferT \<open>''new valuation''\<close> subgoal
          ...
          apply (subst clock_set_set')
          apply (simp add: filter_concat comp_def)
          subgoal ps_neq_empty \<comment> \<open>ps \<noteq> []\<close>
            sorry
          apply (rule clock_set_set_cong)
          using ps_neq_empty apply simp
        done
      done
      done
...
next
  ...
  proof cases
    ...
    have 2: "filter (\<lambda>x. x \<noteq> urge) r = filter (\<lambda>x. x \<noteq> urge) (tl r)" if "r = urge # tl r" for r
      by (subst that) simp
    have 3: "urge \<in> set xs" if "xs = urge # ys" for xs ys
      using that by auto
    from steps(3,2) have "u' urge = 0"
      ...
      subgoal sync for sync syncs' ps N as bs gs fs rs ls' B
        apply (tag- "''new valuation''" "TRANS _"  "SEL ''range''")
        apply (subgoal_tac "ps \<noteq> []")
         apply (simp add: subset_nat_0_atLeastLessThan_conv)
         apply (rule clock_set_set; auto elim!: trans_N_urgeE 3 dest!: hd_in_set; fail)
        sorry
      done
    have **: "
    ...
    moreover from steps(3,2) have
      ...
      subgoal sync premises prems[tagged]
        for sync syncs' ps Na as bs gs fs rs ls' B
        using prems apply del_tags
        usingT "SEL ''range''" \<^cancel>\<open>"SEND ''range''"\<close>
        apply (simp add: subset_nat_0_atLeastLessThan_conv)
        apply (rule step_u.intros)
        preferT \<open>''sync''\<close>
          apply (tag, simp; fail)
        ...
        preferT \<open>''maximal''\<close> subgoal maximal
          by (tag, simp, meson trans_N_urgeD trans_urge_upd_iff)
        preferT \<open>''new valuation''\<close> subgoal new_valuation premises prems[tagged]
          ...
          apply (force elim!: trans_N_urgeE simp: urge_not_in_resets' filter_id_conv 2)
          done
...
lemma mem_trans_N_iff:
  "t \<in> Generalized_Network_Language.trans (N i) \<longleftrightarrow> t \<in> set (fst (snd (snd (automata ! i))))"
  if "i < n_ps"
...
lemma N_p_trans_eq:
  "Generalized_Network_Language.trans (N p) = set (fst (snd (snd (automata ! p))))" if "p < n_ps"
...
lemma var_set_compute:
  ...
  (\<Union>S \<in> (\<lambda>t. (fst \<circ> snd o snd \<circ> snd \<circ> snd) ` set t) ` ((\<lambda>(_, _, t, _). t) `  set automata).
    \<Union>f \<in> S. \<Union> ((x, e), i) \<in> set f. {x} \<union> vars_of_exp e)"
  unfolding var_set_def setcompr_eq_image
  \<^cancel>\<open>by (rule arg_cong2[where f = "(\<union>)"]; auto simp: N_p_trans_eq n_ps_def,
     (drule nth_mem, erule bexI[rotated],
      metis (no_types, lifting) insert_iff mem_case_prodI prod.collapse)+,
      (drule mem_nth, force)+)\<close>
  apply (rule arg_cong2[where f = "(\<union>)"]; auto simp: N_p_trans_eq n_ps_def)
  apply (drule nth_mem, erule bexI[rotated], metis prod.collapse)
  apply (drule mem_nth, force)
  subgoal for p
    by (drule nth_mem, cases "automata ! p"; force)
  subgoal for _ p
    by (drule nth_mem, cases "automata ! p"; force)
  apply (drule mem_nth, force)+
  done
...
locale Generalized_Network_Rename_Defs_real =
  Generalized_Network_Rename_Defs automata +
  Generalized_Network_Impl_real automata
  for automata ::
    "('s list \<times> 's list
...
sublocale renum: Generalized_Network_Impl_real
  "map_index renum_automaton automata"
  "map renum_sync syncs"
  "map (\<lambda>(a,p). (renum_vars a, p)) bounds'"
  .
...
locale Generalized_Network_Rename' =
  Generalized_Network_Rename_Defs where automata = automata for automata ::
    "('s list \<times> 's list
...
lemma inv_renum_sem_I:
  assumes
    "u \<turnstile> Generalized_Network_Language.inv (N p) l" "p < n_ps" "l \<in> loc_set"
  shows
    "map_u u \<turnstile> Generalized_Network_Language.inv (renum.N p) (renum_states p l)"
  using assms
  unfolding inv_def
  apply -
...
lemma inv_renum_sem_D:
  assumes
    ...
    "p < n_ps" "l \<in> loc_set"
  shows
    "u \<turnstile> Generalized_Network_Language.inv (N p) l"
  using assms
  unfolding inv_def
  apply -
...
lemma trans_N_renumD:
  assumes "(l, b, g, a, f, r, l') \<in> Generalized_Network_Language.trans (N p)" "p < n_ps"
  shows
    "(renum_states p l, renum_bexp b, renum_cconstraint g, renum_act a, renum_upds f,
      renum_reset r, renum_states p l') \<in> Generalized_Network_Language.trans (renum.N p)"
  using assms
...
\<comment> \<open>It seems like eta-contractions prevent us from deriving this from @{thm inj_pair} directly.\<close>
lemma inj_case_prod_Pair:
  "inj (case_prod (\<lambda>(a, b). Pair (f a, g b)))" if "inj f" "inj g"
...
lemma injective_functions:
  ...
  subgoal
    ...
    by (intro inj_case_prod_Pair exp.inj_map)
  subgoal
...
lemma trans_sem_N_renumI':
  ...
  shows "\<exists> b' g' a' f' r' l1.
    TRANS ''orig'' \<bbar> (l, b', g', a', f', r', l1) \<in> Generalized_Network_Language.trans (N p)
...
definition "renum_upd' \<equiv> (\<lambda>(x, upd). (renum_vars x, renum_exp upd))"

lemma is_upd_renumD:
  assumes "is_upd s f s'"
  shows "is_upd (s o vars_inv) (renum_upd' f) (s' o vars_inv)"
  using assms unfolding is_upd_def renum_upd'_def
...
lemma sort_upds_renum_upds_eq:
  ...
  by (simp split: prod.splits add: sort_key_map_commute[where k' = snd])

lemma is_upd_idxs_renumD:
  assumes "is_upd_idxs s f s'"
  shows "is_upd_idxs (s o vars_inv) (renum_upds f) (s' o vars_inv)"
  using assms
  unfolding is_upd_idxs_def sort_upds_renum_upds_eq by (rule is_upds_renumD)

lemma sort_upds_concat_map_sort_upds_eq:
  "sort_upds (concat_map renum_upds ps) = map renum_upd' (sort_upds (concat ps))"
  unfolding sort_upds_def
  by (metis (mono_tags, lifting) map_concat sort_upds_def sort_upds_renum_upds_eq)

lemma is_upds_idxs_renumD:
  assumes "is_upds_idxs s1 ps s'"
  shows "is_upds_idxs (s1 o vars_inv) (map renum_upds ps) (s' o vars_inv)"
  using assms unfolding is_upds_idxs_def sort_upds_concat_map_sort_upds_eq by (rule is_upds_renumD)
...
lemma renum_label_Sync_eq:
  "renum_label (Sync sync) = Sync (renum_sync sync)"
  unfolding renum_label_def renum_sync_def by (simp split: prod.split)

lemma renum_syncE:
  assumes "(q, a, weak) \<in> set (renum_sync sync)"
  obtains a' where "(q, a', weak) \<in> set sync" "a = renum_acts a'"
  using assms unfolding renum_sync_def by auto

lemma map_fst_renum_sync_eq:
  "map fst (renum_sync a') = map fst a'"
  by (simp add: renum_sync_def split: prod.splits)

lemma fst_set_renum_sync_eq:
  "fst ` set (renum_sync a') = fst ` set a'"
  by (force simp add: renum_sync_def split: prod.splits)
...
lemma step_single_renumD:
  ...
  subgoal for a'
    ...
    preferT \<open>''new valuation''\<close>
     ...
     preferT \<open>''is_upd''\<close> subgoal for _ _ _ f
      by (tag, erule is_upd_idxs_renumD)
  ...
  subgoal for a'
    ...
    apply (subst (asm) A_split)
    apply (subst renum.A_split)
    apply (simp only: renum_label_Sync_eq)
    apply (erule step_u_elims')
    apply simp
    ...
    preferT \<open>''sync''\<close> subgoal
      unfolding renum.syncs_def by (tag, force simp: set_map renum_sync_def syncs_def)
    ...
    preferT \<open>''maximal''\<close> subgoal
      ...
      apply (fastforce
          dest!: map_u_renum_cconstraint_clock_valD ComD2[OF inj_renum_acts] check_bexp_renumI
          elim!: renum_syncE)
      done

    preferT \<open>''target invariant''\<close> subgoal for ps as bs gs fs rs ls'
      ...
      subgoal
        ...
        subgoal
          by (rule sem_states_loc_setD, simp)
        subgoal
          by blast
        done
      subgoal
    ...
    preferT \<open>''upds''\<close>
      apply (tag, drule is_upds_idxs_renumD, simp add: comp_def; fail)
    preferT \<open>''bounded''\<close>
      apply (tag, erule bounded_renumI'; fail)
    preferT \<open>SEL ''sublist''\<close>
      apply (tag; simp add: map_fst_renum_sync_eq; fail)
    preferT \<open>''only syncs''\<close>
       apply (tag, simp add: fst_set_renum_sync_eq; fail)
    apply (tag; auto elim: renum_syncE; fail)+
    done
  done
...
lemma is_upds_renumI':
  assumes "is_upds (s o vars_inv) (map renum_upd' f) s'"
...
lemma is_upds_renumI'':
  ...
  using assms
  by (induction "map renum_upd' ps" s' arbitrary: ps)
     (auto 4 3 intro: is_upd_renumI'' is_upds.intros)

lemma is_upds_concat_renumI'':
  assumes "is_upds s (concat_map (map renum_upd') ps) s'"
  shows "is_upds (s o renum_vars) (concat ps) (s' o renum_vars)"
  apply (rule is_upds_renumI'')
  using assms by (simp add: map_concat)

lemma is_upd_idxs_renumI':
  assumes "is_upd_idxs (s o vars_inv) (renum_upds f) s'"
  shows "is_upd_idxs s f (s' o renum_vars)"
  using assms
  unfolding is_upd_idxs_def sort_upds_renum_upds_eq
  thm is_upds_renumI'
  apply -
  apply (rule is_upds_renumI')
  .

lemma is_upds_idxs_renumI':
  assumes "is_upds_idxs (s1 o vars_inv) (map renum_upds ps) s'"
  shows "is_upds_idxs s1 ps (s' o renum_vars)"
  using assms unfolding is_upds_idxs_def sort_upds_concat_map_sort_upds_eq by (rule is_upds_renumI')

lemma is_upd_idxs_dom:
  "dom s' = dom s" if "is_upd_idxs s f s'" "\<forall>((x, e), i) \<in> set f. x \<in> dom s"
  using that unfolding is_upd_idxs_def sort_upds_def by (intro is_upds_dom) auto+

lemma is_upds_idxs_dom:
  "dom s' = dom s" if "is_upds_idxs s upds s'" "\<forall>f \<in> set upds. \<forall>((x, e), i) \<in> set f. x \<in> dom s"
  using that unfolding is_upds_idxs_def sort_upds_def by (intro is_upds_dom) auto+
...
lemma sem_trans_upd_domD:
  ...
  shows "(fst o fst) ` set f \<subseteq> var_set"
proof -
  from assms have "(fst o fst) ` set f \<subseteq> var_set"
...
lemma ComD:
  fixes map_action
  assumes "Com a = map_act map_action a1"
  obtains a' where "a1 = Com a'" "a = map_action a'"
  using assms by (cases a1) auto

lemma renum_sem_syncs_eq:
  "renum.syncs = renum_sync ` set syncs"
  unfolding renum.syncs_def by simp

lemma inj_eq_iff:
  "f x = f y \<longleftrightarrow> x = y" if "inj f"
  using that unfolding inj_def by auto

lemma Com_eq_renum_actE:
  assumes "Com (as p) = renum_act a"
  obtains a' where "as p = renum_acts a'" "a = Com a'"
  using assms unfolding renum_act_def by (cases a) auto

lemma trans_sem_N_renum_ComI:
  assumes [tagged]:
    "TRANS ''sync'' \<bar> \<forall>p\<in>set ps.
      (renum_states p (L ! p), bs p, gs p, Com (as p), fs p, rs p, ls p) \<in> trans (renum.N p)"
    ...
    "''sync'' \<bar> sync \<in> renum.syncs"
  obtains bs' gs' fs' rs' ls' as' sync' where
    "TRANS ''sync'' \<bar>
    ...
    "\<forall>(p, a, _) \<in> set sync'. as p = renum_acts (as' p)"
    "\<forall>p \<in> set ps. Com (as p) = renum_act (Com (as' p))"
    ...
    "''enabled'' \<bar> \<forall>(p, a, b) \<in> set sync'. b \<longrightarrow> p \<in> set ps"
    "''actions'' \<bar> \<forall>(p, a, _) \<in> set sync'. as' p = a"
    "''sync'' \<bar> sync' \<in> set syncs"
    "sync = renum_sync sync'"
    "map fst sync = map fst sync'"
proof -
  define t where
    "t p \<equiv> SOME (b', g', a', f', r', l1). (L ! p, b', g', Com a', f', r', l1) \<in> trans (N p)
    \<and> bs p = renum_bexp b' \<and> gs p = renum_cconstraint g' \<and> as p = renum_acts a'
    \<and> fs p = renum_upds f' \<and> rs p = renum_reset r' \<and> ls p = renum_states p l1" for p
  ...
  have *: "case t p of (b', g', a', f', r', l1) \<Rightarrow>
    (L ! p, b', g', Com a', f', r', l1) \<in> trans (N p)
    \<and> bs p = renum_bexp b' \<and> gs p = renum_cconstraint g' \<and> as p = renum_acts a'
    \<and> fs p = renum_upds f' \<and> rs p = renum_reset r' \<and> ls p = renum_states p l1"
    if "p \<in> set ps" for p
    usingT \<open>TRANS _\<close>
    apply -
    apply (drule bspec[OF _ that])
    apply (drule trans_sem_N_renumI')
    subgoal
      usingT \<open>SEL ''range''\<close> using that by auto
    unfolding t_def by (untag, elims add: Com_eq_renum_actE, fo_rule someI, auto)

  obtain sync' where "sync = renum_sync sync'" and [tag \<open>''sync''\<close>]: "sync' \<in> set syncs"
    usingT \<open>''sync''\<close> unfolding renum_sem_syncs_eq image_iff by blast

  define as'' where "as'' p \<equiv> SOME a. \<exists>weak. (p, a, weak) \<in> set sync' \<and> as p = renum_acts a" for p

  have **: "\<exists>weak. (p, as'' p, weak) \<in> set sync' \<and> as p = renum_acts (as'' p)"
    if "(p, a, weak) \<in> set sync'" for p a weak
    unfolding as''_def usingT \<open>''actions''\<close> unfolding \<open>sync = _\<close> renum_sync_def using that
    by - (rule someI_ex, auto)

  have as_as''[simp]: "as p = renum_acts (as'' p)" if "(p, a, weak) \<in> set sync'" for p a weak
    using ** that by auto

  have as''_as'[simp]: "as'' p = as' p" and as_as'[simp]: "as p = renum_acts (as' p)"
    if "p \<in> set ps" for p
  proof -
    from \<open>p \<in> _\<close> obtain a weak where "(p, a, weak) \<in> set sync"
      usingT \<open>SEL ''sublist''\<close> by (fastforce dest!: subseq_subsetD)
    then obtain a' where "(p, a', weak) \<in> set sync'" "a = renum_acts a'"
      unfolding \<open>sync = _\<close> renum_sync_def by auto
    from **[OF \<open>_ \<in> set sync'\<close>] *[OF \<open>p \<in> set ps\<close>] show
      "as'' p = as' p" and "as p = renum_acts (as' p)"
      unfolding as'_def by (auto dest: inj_renum_acts[THEN injD])
  qed

  show ?thesis
    apply (rule that[of bs' gs' as'' fs' rs' ls' sync'])
    unfolding bs'_def gs'_def fs'_def rs'_def ls'_def
    by (untag; (intros, frule *)?; auto simp: as'_def renum_act_def; fail)+
       (tag, auto dest!: inj_renum_acts[THEN injD] simp: \<open>sync = _\<close>renum_sync_def; fail)+
qed
...
lemma inv_sem_N_renum_syncI:
  assumes
  "\<forall>pa<n_ps.
    [concat (map rs ps)\<rightarrow>0]map_u u
      \<turnstile> inv (renum.N pa)
          ((fold (\<lambda>p L. L[p := ls p]) ps (map_index renum_states L)) ! pa)"
  "\<forall>p\<in>set ps. rs p = renum_reset (rs' p)"
  "\<forall>p\<in>set ps. ls p = renum_states p (ls' p)"
  "\<forall>p\<in>set ps. ls' p \<in> (\<Union>(l, b, g, a, r, u, l') \<in> trans (N p). {l, l'})"
  "L \<in> states"
shows
  "\<forall>pa<n_ps. [concat (map rs' ps)\<rightarrow>0]u \<turnstile> inv (N pa) ((fold (\<lambda>p L. L[p := ls' p]) ps L) ! pa)"
proof -
  have [simp]: "concat (map rs ps) = renum_reset (concat (map rs' ps))"
  ...
  have [simp]:
    "((fold (\<lambda>p L. L[p := ls p]) ps (map_index renum_states L)) ! q)
    = renum_states q ((fold (\<lambda>p L. L[p := ls' p]) ps L) ! q)"
    if "q < n_ps" for q
    using assms(4-) that
    by (simp add: assms(3) map_trans_broad_aux1[symmetric] fold_upds_aux_length cong: fold_cong)
  have "(fold (\<lambda>p L. L[p := ls' p]) ps L) ! q \<in> loc_set" if "q < n_ps" for q
    using assms(4-)
...
lemma trans_sem_upd_domI:
  ...
  shows "\<forall>((x, _), _)\<in>set (renum_upds f'). x \<in> dom (s o vars_inv)"
...
lemma renum_syncI:
  assumes "(q, a, weak) \<in> set sync"
  shows "(q, renum_acts a, weak) \<in> set (renum_sync sync)"
  using assms unfolding renum_sync_def by (auto split: prod.splits simp: image_iff)

lemma step_single_renumI[untagged]:
  assumes
    "step_u renum.sem
      (map_index renum_states L) (s o vars_inv) (map_u u) a L' s' u'"
    "L \<in> states" "dom s \<subseteq> var_set" "dom s = var_set"
  shows "\<exists> a1 L1 s1 u1. step_u sem L s u a1 L1 s1 u1
    \<and> ''label eq'' \<bbar> renum_label a1 = a
    \<and> ''new loc'' \<bbar> L' = map_index renum_states L1
    \<and> ''new state'' \<bbar> s' = s1 o vars_inv
    \<and> ''new valuation'' \<bbar> u' = map_u u1"
  using assms(1-3)
  supply [simp] = length_automata_eq_n_ps L_len
  supply [simp del] = map_map_index set_map
  apply (subst A_split)
  ...
  subgoal for a'
    ...
    preferT \<open>''bounded''\<close> subgoal
      ...
      subgoal
        ...
        using assms(4)
        apply (fastforce simp: image_subset_iff)
        done
...
(* or: auto *)
  ...
  subgoal for a'
    apply (simp only:)
    apply (erule step_u_elims')
    subgoal premises prems[tagged] for ps as bs gs fs rs ls'
      using prems(1-3) \<comment> \<open>All those, which are not tagged\<close>
      usingT "SEL ''range''"

      apply (simp add: subset_iff)
      apply (rule trans_sem_N_renum_ComI, (tag, simp; fail)+)

      subgoal for bs' gs' fs' rs' ls'a as' sync'

        apply intros
        apply (rule step_u.intros(3)[where ps = ps and as = as' and sync = sync'])
        ...
        preferT \<open>''maximal''\<close> subgoal
          apply (tag, fastforce dest: renum_syncI simp: renum_act_def
                  dest!: trans_N_renumD map_u_renum_cconstraint_clock_valI check_bexp_renumD)
          done

        preferT \<open>''target invariant''\<close>
          apply (tag "''new loc''" "''new valuation''" "TRANS ''sync''")
          apply (simp, erule (2) inv_sem_N_renum_syncI, blast, solve_triv; fail)

        preferT \<open>''upds''\<close>
          apply (tag, rule is_upds_idxs_renumI', simp cong: map_cong; fail)

        preferT \<open>''bounded''\<close> subgoal
          using \<open>dom s = var_set\<close> apply (tag "''upds''" "TRANS ''sync''")
          apply (erule bounded_renumD1)
          apply (drule is_upds_idxs_dom)
          subgoal
            apply (subst set_map, simp)
            apply (erule Ball_mono, rule trans_sem_upd_domI[simplified], erule trans_N_renumI)
        ...
        preferT \<open>''only syncs''\<close>
          apply (tag, simp add: fst_set_renum_sync_eq; fail)
        preferT \<open>''label eq''\<close>
          apply (tag, metis renum_label_Sync_eq renum_label_def)
        preferT \<open>''new loc''\<close> apply (tag, solve_triv; fail)
        preferT \<open>''new loc''\<close>
          apply (tag, simp add: map_trans_broad_aux1 map_index_update cong: fold_cong; fail)
        preferT \<open>''new state''\<close>
           apply (tag, auto simp: vars_inv_def bij_f_the_inv_f[OF bij_renum_vars]; fail)
        preferT \<open>''new valuation''\<close> apply (tag, solve_triv; fail)
        preferT \<open>''new valuation''\<close> apply (tag,
          simp add: renum_reset_map_u[symmetric] renum_reset_def map_concat cong: map_cong; fail)
        apply (tag; solve_triv)+
        done
      done
    done
...
lemma n_ps_eq1:
  "Prod_TA_Defs.n_ps
        (set x, map automaton_of (map conv_automaton automata),
         map_of bounds') = n_ps"
  unfolding n_ps_def Prod_TA_Defs.n_ps_def by simp

lemma var_set_eq1:
  "Prod_TA_Defs.var_set
     (set syncs, map automaton_of (map conv_automaton automata),
...
sublocale real: Generalized_Network_Rename_real where automata = "map conv_automaton automata"
  apply standard
...
lemma sem_unfold2:
  ...
  by (simp add: Generalized_Network_Impl.sem_def rename_conv_automaton_commute)
...
locale Generalized_Network_Rename_Start' =
  Generalized_Network_Rename' where automata = automata
  for automata ::
    "('s list \<times> 's list
      \<times> (('a :: countable) act, 's, 'c, 't, 'x :: countable, int) transition list
...
locale Generalized_Network_Rename_Start_int =
  Generalized_Network_Rename_int where automata = automata +
  Generalized_Network_Rename_Start' where automata = automata
  for automata ::
    "('s list \<times> 's list
      \<times> (('a :: countable) act, 's, 'c, int, 'x :: countable, int) transition list
      \<times> (('s :: countable) \<times> ('c :: countable, int) cconstraint) list) list"
...
end (* Generalized Network Rename Start int *)

locale Generalized_Network_Rename_Formula_int =
  Generalized_Network_Rename_Start_int where automata = automata
  for automata ::
    "('s list \<times> 's list
      \<times> (('a :: countable) act, 's, 'c, int, 'x :: countable, int) transition list
      \<times> (('s :: countable) \<times> ('c :: countable, int) cconstraint) list) list" +
...
lemma (in Generalized_Network_Impl) act_set_compute:
  "act_set =
  \<Union> ((\<lambda>(_, _, t, _). \<Union> ((\<lambda>(l, e, g, a, _). act.set_act a) ` set t)) ` set automata)
  \<union> (\<Union>sync \<in> set syncs. (fst o snd) ` set sync)"
  unfolding act_set_def
  apply (fo_rule arg_cong2)
  apply (auto simp: N_p_trans_eq n_ps_def act_set_def syncs_def)
     apply (drule nth_mem, erule bexI[rotated], force)+
   apply (drule mem_nth, force)+
  done
...
lemma (in Generalized_Network_Impl)
  ...
  shows clk_set'I1[intro]: "c \<in> set r \<Longrightarrow> c \<in> clk_set'"
    ...
    and var_setI2[intro]: "((x, e), i) \<in> set f \<Longrightarrow> x \<in> var_set"
    and var_setI3[intro]: "((x, e), i) \<in> set f \<Longrightarrow> v \<in> set_exp e \<Longrightarrow> v \<in> var_set"
...
lemma (in Generalized_Network_Impl) clk_set'I3[intro]:
...
locale Generalized_Network_Rename_Defs_int_urge =
...
lemma (in Generalized_Network_Rename_Defs) conv_automaton_of:
  "Generalized_Network_Language.conv_A (automaton_of A) = automaton_of (conv_automaton A)"
  unfolding automaton_of_def conv_automaton_def Generalized_Network_Language.conv_A_def
  by (force split: prod.splits
...
lemma act_set_finite:
  "finite act_set"
  unfolding act_set_def syncs_def by (auto intro!: finite_intros)
...
sublocale rename: Generalized_Network_Rename_Defs_int
  syncs bounds'
...
sublocale rename: Generalized_Network_Rename_int
  syncs bounds'
  "extend_bij renum_acts act_set"
  "extend_bij renum_vars var_set"
  "extend_bij renum_clocks (insert urge clk_set')"
...
lemma syncs_act_setI:
  "a \<in> act_set" if "x \<in> set syncs" "(x1, a, ba) \<in> set x"
  using that by (force simp: act_set_def syncs_def)

lemma rename_N_eq_sem:
  "rename.renum.sem =
  Generalized_Network_Language_Model_Checking.N
    (map renum_sync syncs)
    (map_index renum_automaton (map (conv_urge urge) automata))
    (map (\<lambda>(a,p). (renum_vars a,p)) bounds')"
  unfolding rename.renum.sem_def
  unfolding Generalized_Network_Language.conv_def
  apply simp
  apply (intro conjI)
  subgoal
    by (rule image_cong, simp,
        clarsimp simp: rename.real.renum_sync_def renum_sync_def split: prod.splits,
        intro HOL.refl renum_acts_bij_extends syncs_act_setI)
  subgoal
...
lemma conv_automaton_of':
  "automaton_of \<circ> conv_automaton = Generalized_Network_Language.conv_A o automaton_of"
...
end (* Generalized Network Rename *)
...
sublocale rename: Generalized_Network_Rename_Start_int
  syncs bounds'
  "extend_bij renum_acts act_set"
  "extend_bij renum_vars var_set"
  "extend_bij renum_clocks (insert urge clk_set')"
...
lemma rename_a\<^sub>0'_eq:
  ...
  subgoal
    ...
    by (auto simp: state_eq Generalized_Network_Rename_Defs.vars_inv_def)
  done
...
lemma N_eq_sem:
  "Generalized_Network_Language_Model_Checking.N syncs automata bounds' = sem"
...
locale Generalized_Network_Rename_Formula =
  Generalized_Network_Rename_Start where automata = automata
  for automata ::
    "('s list \<times> 's list
      \<times> (('a :: countable) act, 's, 'c, int, 'x :: countable, int) transition list
...
sublocale rename: Generalized_Network_Rename_Formula_int
  syncs bounds'
  "extend_bij renum_acts act_set"
  "extend_bij renum_vars var_set"
  "extend_bij renum_clocks (insert urge clk_set')"
...
end (* Generalized_Network_Rename_Formula *)
