...
proof -
  ...
  define P where "P a b \<equiv> a \<in> A \<and> b \<in> B \<and> a \<notin> S \<and> b \<notin> f ` S \<and>
    ...
    for a b
  have finiteI: "finite {x. x \<in> A \<and> ordX x \<le> ordX a \<and> Q x}" (is "finite ?S")
      ...
      proof (cases "ordX x \<le> ordX a")
        ...
        proof (cases "\<exists>b \<in> A. b \<notin> F \<and> ordX a < ordX b \<and> ordX b \<le> ?m")
          ...
          proof -
            have "{xa \<in> A. ordX xa = max ?m (ordX a) + 1 \<and> xa \<notin> insert x F} = {?a}"
              ...
              apply (auto simp add: \<open>inj_on ordX A\<close> the_inv_into_f_eq)
              using True \<open>ordX ?a = _\<close> \<open>?a \<in> A\<close>
                 apply auto
              by (smt Max.in_idem One_nat_def \<open>inj_on ordX A\<close> add.right_neutral add_Suc_right
                  discrete finite_imageI inj_on_image_mem_iff insert.prems(2) insert_subset
                  insert(1) max_def not_less not_less_iff_gr_or_eq)
        ...
        next
          ...
          then have **: "{xa \<in> A. ordX xa \<le> ordX ?a \<and> xa \<noteq> x \<and> xa \<notin> F} =
            ...
            apply auto
            subgoal
              by (smt \<open>inj_on ordX A\<close> a inj_on_eq_iff le_eq_less_or_eq le_trans mem_Collect_eq not_le)
            done
