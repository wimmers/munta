...
subsection \<open>Pre-compiled networks with states and clocks as natural numbers\<close>
  ...
  begin
    ...
    definition N :: "(nat, 'b, nat, int, nat) nta" where "N \<equiv> map (\<lambda> i. (T i, I i)) [0..<p]"
  ...
  locale Network_Reachability_Problem_precompiled = Network_Reachability_Problem_precompiled_defs +
    assumes process_length: "length inv = p" "length trans = p"
        ...
        and trans_complete:
          ...
          "\<forall> q1 < p. \<forall> t1 \<in> T q1. case t1 of (l1, g1, (Out a, b1), r1, l1') \<Rightarrow> \<exists> q2 < p. \<exists> l2 g2 b2 r2 l2'.
            q1 \<noteq> q2 \<and> (l2, g2, (In a, b2), r2, l2') \<in> T q2 | _ \<Rightarrow> True"
  begin
