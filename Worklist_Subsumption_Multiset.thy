...
context Search_Space_Defs begin
  ...
  definition "add_succ_spec wait a \<equiv> SPEC (\<lambda>(wait',brk).
    if \<exists>a'. E a a' \<and> F a' then
      ...
      (\<forall> s \<in> set_mset wait \<union> {a' . E a a'}. \<exists> s' \<in> set_mset wait'. s \<preceq> s')
  )"
...
context Search_Space begin
  ...
  private lemma aux3:
    assumes
      ...
      "a \<in># wait"
      "\<forall> s \<in> set_mset (wait - {#a#}) \<union> Collect (E a). \<exists> s' \<in> set_mset wait'. s \<preceq> s'"
      "worklist_inv_frontier passed wait"
  ...
  proof -
    ...
    unfolding worklist_algo_def add_succ_spec_def F_reachable_def
      ...
      using Un_iff insertI2 insert_DiffM mem_Collect_eq set_mp set_mset_add_mset_insert step_reachable
      apply metis
        (*Variant*)
